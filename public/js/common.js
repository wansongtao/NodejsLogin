...
commonObj.redirectPage = ({
    ...
    if (typeof pageUrl !== 'string' || typeof time !== 'number') {
        throw {
            message: 'serverMessage.js => redirectPage(): arguments type error.'
        };
    }
...
/**
 * @description 弹出询问框
 * @param {string} msg 询问文本
 * @param {Function} callback 点击确定后，执行的函数
 * @param {object} dataObj 传给函数的参数对象  格式：{pageUrl, time}
 */
commonObj.confirmMsg = (msg, callback, dataObj) => {
    if (typeof msg !== 'string' || !(callback instanceof Function) ||
        !(dataObj instanceof Object)) {
        throw {
            message: 'serverMessage.js => confirmMsg(): arguments type error.'
        };
    }

    //弹出询问框，用户点击确定后，执行相应的操作
    layer.confirm(msg, {
        btn: ['确定', '取消'] //按钮
    }, function () {
        callback(dataObj);
